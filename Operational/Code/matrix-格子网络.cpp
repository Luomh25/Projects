...
int evolution(int L = 200) {
    ...
    while(true){
        ...
        for (int _iter = 0; _iter < N; _iter++) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                if(d==1 || d == 3){
                    decision[per][d] = strategy(pay_last, payoffs[nei], stratsW[per], stratsV[per], stratsa[per], stratsb[per], p1(g), p2(g));
                    temp = strategy(payoffs[nei], pay_last, stratsW[nei], stratsV[nei], stratsa[nei], stratsb[nei], p1(g), p2(g));
                ...
                //比较强弱用真实而不是期望
                int tem = E_pay(pay_last,payoffs[nei],stratsW[per], stratsV[per]);
    ...
    int aa=0;
    for (int i = 0; i < N; i ++){
        if (stratsW[i] == 1 && stratsV[i] == 0 && stratsb[i] <= 0.3) ++aa;
    }    
    cout << "第" << count << "个蒙特卡洛步结束" << endl;
    // 关闭文件
    outfile.close();
    cout << "数据已写入output.txt文件" << endl;
    cout << P1_cnt << " " << P2_cnt << " " << P3_cnt << ":" << aa << " " << P4_cnt << endl;
    return 0;
}
